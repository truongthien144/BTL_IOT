...
String mainPage()
{
  ...
  String tempDisp = isWifiConnected ? String(temperature) : String("N/A");
  String humDisp = isWifiConnected ? String(humidity) : String("N/A");
    ...
    <div class="container">
      ...
      <div class="sensor">
        🌡️ Nhiệt độ: <span id="temp">)rawliteral" +
         tempDisp + R"rawliteral(</span> &deg;C
      </div>
      <div class="sensor">
        💧 Độ ẩm: <span id="hum">)rawliteral" +
         humDisp + R"rawliteral(</span> %
      </div>
...
void handleToggle()
{
  ...
  if (led == 1)
  {
    // toggle manual LED1 state and enable manual override
    led1_state = !led1_state;
    led1_manual = true;
    // apply immediately to hardware
    digitalWrite(LED1_PIN, led1_state ? HIGH : LOW);
    Serial.printf("LED1 manual -> %s\n", led1_state ? "ON" : "OFF");
  }
  else if (led == 2)
  {
    // toggle manual NEO state and enable manual override
    led2_state = !led2_state;
    led2_manual = true;
    // neo_blinky task will apply the change on next loop
    Serial.printf("LED2 (neo) manual -> %s\n", led2_state ? "ON" : "OFF");
  }
...
void handleSensors()
{
  if (!isWifiConnected)
  {
    server.send(200, "application/json", "{\"temp\":\"N/A\",\"hum\":\"N/A\"}");
    return;
  }
...
void handleConnect()
{
  // store in canonical globals used across tasks
...
// startAP() is implemented in task_wifi.cpp (shared single definition)
